...
public class ObjectCommonUtil
{
    ...
    for (int i = 0; i < resultMethods.length; i++) {
      ...
      if (setter.startsWith("set")) {
        ...
        try
        {
          ...
          if ((sourceMethod != null) && (sourceMethod.getReturnType() == paramType))
          {
  ...
  public static void printObject(Object object)
  {
    ...
    String getter = null;
    Class paramType = null;
    System.out.println("******************** PRINT OBJECT START *************************");
    for (int i = 0; i < objMethods.length; i++) {
      getter = objMethods[i].getName();
      paramType = objMethods[i].getReturnType();
      if ((paramType == Class.class) || ((!getter.startsWith("get")) && (!getter.startsWith("is"))))
        continue;
      try {
        System.out.println(getter + "==" + objMethods[i].invoke(object, new Object[0]));
    ...
    System.out.println("******************** PRINT OBJECT END *************************");
  }
