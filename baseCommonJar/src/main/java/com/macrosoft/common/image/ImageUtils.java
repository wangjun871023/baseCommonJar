...
public class ImageUtils {
	...
	public static boolean resizeImage(String srcPath, String toPath, int width,
			int height) {
		try {
			...
			int type = getImageTypeBySuffix(filename.substring(
					filename.lastIndexOf("."), filename.length()));
			BufferedImage bufferedImage = resizeImage(ImageIO.read(file), type,
					width, height);
	...
	public static boolean saveImage(BufferedImage image, String toFileName,
			int type) {
		try {
			return ImageIO.write(image, getImageSuffixByType(type), new File(
					toFileName));
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public static boolean saveImage(InputStream stream, String formatName,
			String toPath) {
		try {
			if (toPath == null) {
				throw new IllegalArgumentException("参数异常,toPath=" + toPath);
			...
			if (formatName == null) {
				throw new IllegalArgumentException("参数异常,formatName="
						+ formatName);
			}
			return ImageIO.write(ImageIO.read(stream), formatName, new File(
					toPath));
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public static boolean rotateImage(String srcPath, String toPath, int value) {
		...
		try {
			...
			int type = getImageTypeBySuffix(filename.substring(filename
					.lastIndexOf(".") + 1));
